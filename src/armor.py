...
def decode_block(chars_block, alphabet, *, shift=False):
    ...
    v = bytes_int.to_bytes(bytes_size, byteorder='big')
    print("len(chars_block) {}, bytes_size {} , extra {}, len(v) {}".format(len(chars_block), bytes_size, extra, len(v)))
    print("  ", " ".join("{}".format(c) for c in v))
    return v
...
data = ""


res = dearmor(data, alphabet=b62alphabet, char_block_size=43, raw=True, shift=False)
...
exit(0)
for i in range(1, 33):
    cs = min_chars_size(62, i)
    bs = max_bytes_size(62, cs)
    print("{}, // {} bytes".format(cs, i))
    # print("{} bytes need {} chars which will become again {} bytes".format(i, cs, bs))
    assert(i==bs)

for i in range(0, 44):
    bs = max_bytes_size(62, i)
    cs = min_chars_size(62, bs)
    ex = " (!)" if (i != cs) else ""
    print("{}, // {} chars{}".format(bs, i, ex))
    #print("{} chars will become {} bytes which would become again {} chars".format(i, bs, cs))